    ...
    while (true) {
      ...
      if (answer == "1")
      {
        ...
        else
        {
          ...
          await token.transfer(accounts[Number(answer) - 1].address, ethers.parseUnits("100", 18));
          console.log("\nAccount%d has now : %d JT42T",
            answer, ethers.formatUnits(await token.balanceOf(accounts[Number(answer) - 1].address)));
          console.log("Remining tokens for owner : %d JT42T\n",
            ethers.formatUnits(await token.balanceOf(owner.address)));
        }
      }
      else if (answer == "2")
      {
...
async function bonus() {
  ...
  const [admin1, admin2, admin3, account4, account5, account6,
        account7, account8, account9, account10, account11, account12,
        account13, account14, account15, account16, account17, account18,
        account19, account20
  ] = await ethers.getSigners();

  token.transfer(await multisig.getAddress(), await token.balanceOf(admin1.address));
  token.transferOwnership(await multisig.getAddress());

  const accounts = [admin1, admin2, admin3, account4, account5, account6,
        account7, account8, account9, account10, account11, account12,
        account13, account14, account15, account16, account17, account18,
        account19, account20
  ];
  ...
  console.log("Admin3 address:", admin3.address);
  ...
  while (true) {
    ...
    if (index >= 1 && index <= 3)
    {
      ...
      else if (index == 2)
      {
        ...
        try {
          await multisig.transferToken(await accounts[indexAccount].getAddress(),
      ...
      else if (index == 3)
      {
        ...
        try {
          await multisig.burnToken(await multisig.getAddress(),
            ethers.parseUnits(numberTokens, 18));
        }
        catch (e: any) {
          console.log("\n❌", e.reason ?? e.message ?? "Unknown error");
    ...
    else if (answer == "4")
      {
        ...
        if (answer == "1" || answer == "2" || answer == "3")
          console.log("\nNumber of tokens JT42T for admin%d : %d JT42T\n",
            answer, ethers.formatUnits(await token.balanceOf(accounts[Number(answer)].address)));
        else
        {
          const index = Number(answer);
